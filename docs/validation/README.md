...
### iOS capture fields and fill template

The iOS helper creates `ios-physical-device-evidence-fill-template.json` under the run artifact folder; it is a non-promotable starting shape until real device artifacts and checks are filled. Immediately after selecting Screen Time targets, record `ios.familyActivityPickerAppLimitActivityName=freed.selectedAppDailyLimit` and `ios.familyActivityPickerAppLimitEventName=freed.selectedAppDailyLimitReached` with the local Screen Time report that proves the selected-target daily-limit monitor was scheduled. Camera challenge evidence must name the on-device `Vision labels` used for the submitted photo, together with its bounded confidence and temporary-file cleanup proof; it must not represent a continuous image-classification loop.

Performance rows require local artifact references rather than standalone numbers: `networkSpeedArtifact` must be a sanitized `freed-network-speed-report-v1` report for the same run/platform with before/during Mbps and no-proxy/no-MITM checks, while `dnsLatencyArtifact` must be a sanitized `freed-dns-latency-report-v1` report for the same run/platform with p95 latency, sample count, and DNS-only routing proof.
...
```json
{
  ...
  "ios": {
    ...
    "safariFocusShieldBuildRunId": "<safari-focus-shield-build-run-id>",
    "safariFocusShieldBuildArtifact": "path/to/local-ios-app-package-proof.json",
    "safariFocusShieldShortFormUrl": "https://youtube.com/shorts/dQw4w9WgXcQ",
    "safariFocusShieldShortFormBlockRunId": "<safari-focus-shield-short-form-block-run-id>",
    "safariFocusShieldShortFormBlockArtifact": "path/to/local-safari-focus-shield-short-form-block-report.json",
