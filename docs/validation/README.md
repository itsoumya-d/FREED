...
`npm run evidence:ios-devices` runs the same bounded Xcode device discovery used by the full helper, tries `xcrun xctrace list devices` first, falls back to `xcrun devicectl list devices --json-output` when xctrace hangs or fails, writes `ios-device-discovery.json` plus the successful `xctrace-devices.txt` or `devicectl-devices.json` artifact, and reports whether a physical iPhone name/UDID is ready or only listed offline; it is a setup handoff only and does not mark evidence as passing. `npm run evidence:ios-physical-device` refuses simulator targets and offline/not-ready physical devices unless `--plan-only` is used to create a non-promotable setup packet, then collects physical iPhone metadata and a manual-review manifest for the iOS physical-device gate, including Family Controls entitlement, app-group provisioning, Complete Data Protection entitlement proof, immediate post-picker selected-target daily-limit monitor scheduling, Safari Content Blocker signed-target, synced-feed reload, SafariServices enabled-state, adult-block, web short-form block, and Safari/web short-form challenge-handoff proof rows; pass `--app <signed-freed-app-or-ipa>` to generate `ios-app-package-proof.json`, a local `freed-ios-app-package-proof-v1` JSON report with `sanitized=true`. That report inspects codesign entitlement availability, Family Controls, the shared `group.app.freed.recovery` app group, Complete Data Protection on the app and every embedded extension, embedded Shield Configuration, Shield Action, DeviceActivity Monitor, and Safari Content Blocker extensions, bundled Safari `blockerList.json` adult-domain and YouTube Shorts / Instagram Reels / TikTok For You web-rule signals, all-block actions, and absence of packet-tunnel/packet-inspection entitlements. The validator requires that same local app-package proof for `ios.familyControlsEntitlementArtifact`, `ios.appGroupProvisioningArtifact`, `ios.completeDataProtectionEntitlementArtifact`, and `ios.safariContentBlockerBuildArtifact`, with `packageProofUsableForManualEvidence=true` and empty entitlement/Safari rule failure arrays. The helper also writes `ios-physical-device-evidence-fill-template.json` with concrete run IDs, platform URL/host context, and pending final-shape `ios`/`checks` fields so QA can fill physical-device proof without reconstructing the schema from prose; `--plan-only` writes that fill template, `capture-manifest.json`, and `CAPTURE_NOTES.md` with `evidenceSatisfied=false`. Safari reload, adult-block, and web short-form block artifacts must be local `freed-ios-safari-content-blocker-report-v1` JSON reports with `sanitized=true` matching run ID, version, checksum, rule count, `safariContentBlockerEnabled=true`, adult host or short-form URL/rule, and proving Safari Content Blocker-layer operation with no third-party app screen inspection, no continuous screen reading, no packet tunnel, no packet inspection, and no MITM HTTPS. After tapping Done in FamilyActivityPicker, QA must capture `ios.familyActivityPickerAppLimitScheduledImmediately=true`, `ios.familyActivityPickerAppLimitActivityName=freed.selectedAppDailyLimit`, and `ios.familyActivityPickerAppLimitEventName=freed.selectedAppDailyLimitReached` before relying on the later threshold-fired proof. Prepare App Store review notes from `docs/store-policy/ios-screen-time-safari-dns-review.md` before filling entitlement, provisioning, signed-target, optional DNS Settings, or physical-device behavior artifacts. The final evidence must also include separate physical-device challenge verification proof for fresh-camera Vision labels, no base64/EXIF media payload, temporary photo cleanup, motion samples, pedometer steps, and accurate foreground location fixes. The iOS challenge proof artifacts must be local JSON reports with `sanitized=true`: `freed-challenge-photo-report-v1`, `freed-challenge-motion-report-v1`, `freed-challenge-steps-report-v1`, and `freed-challenge-location-report-v1`; their run IDs, platform, metrics, on-device/on-demand checks, timer-only bypass rejection, no-retention checks, and foreground/no-raw-coordinate location proof must match the evidence row. It refuses simulator targets, placeholder/local/insecure normal URLs, placeholder/local adult hosts, and non-short-form `--short-form-url` values, and does not mark evidence as passing.
...
`routingProofArtifact` must be a local `freed-routing-proof-report-v1` JSON report with `sanitized=true` whose `runId`, `platform`, and `protectionMode` match the row and whose checks prove no full-traffic proxy, no packet inspection, no MITM HTTPS, no continuous packet capture, and unaffected normal browsing routes. Android routing reports must also prove DNS-only VPN service state plus Private DNS, VPN, proxy, and route-table capture with no remote traffic tunnel; iOS routing reports must prove Screen Time/ManagedSettings/Safari or matched-domain DNS Settings boundaries with no packet tunnel or NetworkExtension packet tunnel. `networkSpeedArtifact` must be a local `freed-network-speed-report-v1` JSON report with `sanitized=true` whose `runId`, `platform`, `metrics.downloadMbpsBefore`, and `metrics.downloadMbpsDuring` match the row and whose checks prove same-network testing plus no full-traffic proxy, packet inspection, or MITM HTTPS. `dnsLatencyArtifact` must be a local `freed-dns-latency-report-v1` JSON report with `sanitized=true` whose `runId`, `platform`, `metrics.dnsLatencyP95Ms`, and positive `metrics.sampleCount` match the row and whose checks prove DNS latency stayed within threshold under DNS-only routing.
